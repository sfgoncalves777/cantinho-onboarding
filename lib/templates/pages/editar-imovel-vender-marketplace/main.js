const urlBaseUpdateProperty = '{{urlBaseUpdateProperty}}';
const urlBaseFinishProperty = '{{urlBaseFinishProperty}}';
const urlBaseDeleteProperty = '{{urlBaseDeleteProperty}}';
const urlBaseGetProperty = '{{urlBaseGetProperty}}';
...
const handleImageChange = (event, fieldId) => {
  ...
  if (file) {
    ...
    reader.onload = () => {
      if (imageUrls.includes(preview.src)) {
        const updateImageUrls = imageUrls.filter(url => url !== preview.src);
...
const submitForm = async (event) => {
  ...
  const resUpdatedProperty = await fetch(`${urlBaseUpdateProperty}?propertyId=${propertyId}`, {
    method: 'PUT',
  ...
  const resFinishProperty = await fetch(`${urlBaseFinishProperty}?propertyId=${propertyId}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
...
const deleteProperty = async () => {
  ...
  const resDeleteProperty = await fetch(`${urlBaseDeleteProperty}?propertyId=${propertyId}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const resGetProperty = await fetch(`${urlBaseGetProperty}?propertyId=${propertyId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
